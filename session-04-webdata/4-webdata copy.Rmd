...
stringr::str_replace_all(words, c("ing" = "er"))
```
:::
...
Answer: Only names, one or more letters then might or might not have space and/or comma, then one or more letters wl might or might not period. Only number, 3 digits, wither dash or space, then might or might not have another 3 digits plus a dash than 4 digits. 
:::
...
| Specification | Meaning | Shorthand version |
  ...
  | `[:alpha:]`  | Alphabetic characters: a-z and A-Z | \\w | *Actually \\w is not shorthand for letters coz it's word character including digits, it could be shorthand for [:alnum:]*
...
```{r}
str_extract(example.obj2, "([[:alpha:]]).+?\\1")
example.obj2 <- "1. A small sentence. - 2. Another tiny sentence. A" 
...
1. Can you explain what these regular expressions match?
      ...
      Answer: price 
      a. `"^.*$"`
      Answer: anything 0 or more times 
      a. `"\\d{4}-\\d{2}-\\d{2}"`
      Answer: 2018-05-19
      a. `".*?\\.txt$"`
      Answer:any text file
      a. `"\\\\{4}"`
      Answer: four backlashes
      a. `"b[a-z]{1,4}"`
      Answer: a words starts with b with 1-4 letters like baba
...
str_extract_all(
  emails,
  pattern = "([[:alnum:].]+)@students\\.hertie-school\\.org"
)
...
str_c(str_extract_all(secret, "[A-Z]")[[1]], collapse = "")
