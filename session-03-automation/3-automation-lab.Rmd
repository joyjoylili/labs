...
penguins_nested <- penguins_raw %>%
  group_by(Species) %>%
  mutate( Bill_Length = `Culmen Length (mm)` , # simplify column name for later task
    Species = case_when( #simplifing species names 
...
summarize_bills <- function(df) { 
  # Your code here: use summarise() to create the three variables
  df %>%
  summarise(
  count = nrow(df),
  mean_bill_length = mean(Bill_Length, na.rm = T),
  max_bill_length = max(Bill_Length, na.rm = T)
)
}
...
mutate(
 bill_stats = map(data, summarize_bills)
  )
# Look at the result
penguins_nested
```
...
```{r, echo= FALSE, eval= FALSE}
final_summary <- penguins_raw %>%
  select(Species, `Culmen Length (mm)`) %>%
  rename(bill_length = `Culmen Length (mm)`) %>% 
  group_by(Species) %>%
  nest() %>% 
  mutate(bill_stats =
           map(data,
...
#trying to replicate the results without nesting or function...
final_summary2 <- penguins_raw %>%
  select(Species, `Culmen Length (mm)`) %>%
  rename(bill_length = `Culmen Length (mm)`) %>% 
  group_by(Species) %>%
  summarise(
...
#so I literally don't see the point of nesting..??? 
```
:::
...
*Answer: yes coz script 3 imported the clean data...so without running script 3 first, script 4 wouldn't run but I guess can also juts combine the two? *
...
cron_add(
  command = cmd, #is the path to Master R script defined above
  frequency = "daily",
  at = "01:00",
  id = "lotr_pipeline_daily",
  description = "Daily LOTR analysis pipeline",
  ask = FALSE #prevents the "Are you sure?" prompt
)
...
```{r eval = FALSE}
geod_dist <- function(lat1, lon1, lat2, lon2, earth_radius = 6371) {
