...
You need to identify flights operated by planes that **aren't in the registration database**. This could indicate:
- Data entry errors (wrong tail numbers)
- Foreign-registered aircraft
- Missing records that need to be added to the system

**Your task:** 

a) Find all flights where the plane's `tailnum` doesn't appear in the `planes` registry. How many flights were operated by unregistered aircraft?
...
```{r}
#a) & b)
unregistered_flights <- flights %>% 
  anti_join(planes, by = "tailnum") 

unregistered_flights %>% 
  group_by(carrier) %>%
  summarise(number = n()) %>%
  arrange(desc(number))%>%
  slice_head(n = 1)
...
dplyr::copy_to(
  ...
  name = "airports", # we create the "weather" table
  temporary = FALSE, 
  indexes = list(
    c("name", "lat", "lon", "alt")
  )
)
airports_db <-  dplyr::tbl(con, 'airports')
...
Jan1_df <- flights_db %>% left_join(airlines_db, by = "carrier") %>% 
  mutate(airline_name = name) %>%
  filter(month == "1",
         day == "1") %>%
  select(carrier, airline_name, flight) %>%
  collect()
